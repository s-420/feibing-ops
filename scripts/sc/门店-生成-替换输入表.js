 ...
 * @动作    导出
 * @风险    低
 * @来源    store-data-extractor/ShengChengTiHuanShuRuBiao.js
 ...
 *       按「三方ID」匹配出店铺ID，生成 TiHuanMenDianRuQunLianJie.js 需要的
 *       三列表格（门店名称 / 店铺ID / 入群链接）。
 *
 * 纯本地 Excel 操作，不调用 API，无需 token。
 * 匹配键优先级：三方ID > 三方门店ID。
 *
 * 用法：改下方 config 里的两个输入路径，然后 node 运行即可。
 */
...
const config = {
  ...
  // 输出（替换脚本的输入表，列名必须精确为「门店名称 / 店铺ID / 入群链接」）
  outputPath: path.join(__dirname, '../input', '门店ID和入群链接_茶瀑布.xlsx'),
  // 未匹配门店清单
  unmatchedPath: path.join(__dirname, '../output', '未匹配门店.xlsx'),
};
...
function main() {
  ...
  const cLink = colIndex(chead, config.clientLinkCol);
  ...
  for (const r of crows.slice(1)) {
    ...
    if (shopId) {
      matched.push({ name, shopId, link, via });
    } else {
      unmatched.push({ name, thirdId, thirdShopId });
    }
  }
  ...
  const outRows = matched.map(m => ({ '门店名称': m.name, '店铺ID': m.shopId, '入群链接': m.link }));
  ...
  if (unmatched.length > 0) {
    const uRows = unmatched.map(u => ({ '门店名': u.name, '三方id': u.thirdId, '三方门店ID': u.thirdShopId }));
    ...
    console.log('--- 未匹配门店 ---');
    unmatched.forEach(u => console.log(`  ${u.name} | 三方id=${u.thirdId} | 三方门店ID=${u.thirdShopId}`));
  }
}
