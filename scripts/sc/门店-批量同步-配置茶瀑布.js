 ...
 * @动作    批量写
 * @风险    高
 * @来源    store-data-extractor/PiLiangTongBuMenDianPeiZhiChaPuBu.js
...
const API_TEMPLATES = {
  ...
  getStoreConfigId: 'https://vinci-api.feibing.tech/sc/v1/sellers/wpV3cNDAAA3ZRQDQZt0UsR9wORkx0thg/configs?type=MINI_APP_STYLE_TYPE&key={storeKey}',
  // 3. 更新门店配置的接口（末尾ID=查询到的配置ID）
  updateStoreConfig: 'https://vinci-api.feibing.tech/sc/v1/sellers/wpV3cNDAAA3ZRQDQZt0UsR9wORkx0thg/configs/{configId}'
};

// Excel配置
const EXCEL_CONFIG = {
  path: path.join(__dirname, '../input/同步企业设置_茶瀑布.xlsx'), // 根目录input表格
  storeIdColumnName: '门店ID' // 确认Excel列名
};

// 固定并发池配置：任意门店完成后立即补充下一条
...
/**
 * 固定并发池：任一任务完成后立即领取下一条，并保持结果顺序与输入一致
 */
async function runWithConcurrency(items, concurrency, worker, delayMs = 0) {
...
async function main() {
  try {
    ...
    // 阶段2：固定并发池处理门店（查ID→更配置）
    console.log(`✅ 【步骤3/3】开始批量处理门店配置（并发数：${CONCURRENCY}）...\n`);
    const results = await runWithConcurrency(
      storeIds,
      CONCURRENCY,
      async (storeId, index) => {
        console.log(`=====================================================`);
        console.log(`📌 正在处理第 ${index + 1}/${storeIds.length} 个门店：${storeId}`);
        console.log(`=====================================================`);
        return updateSingleStoreConfig(storeId, defaultValue);
      },
      REQUEST_DELAY
    );
